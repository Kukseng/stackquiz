...
// // Avatar selector (client-only)
// const AvatarSelector = dynamic(() => import('./AvatarSelector'), { ssr: false })
...
//               try { onTimeUpRef.current?.(timeUpMessage) } catch (e) { console.warn('onTimeUp handler failed', e) }
//             }
//           }),
...
//       }
//     },
//     // Time-up callback (called by websocket hook)
//     (timeUpPayload) => {
//       console.log("⏰ Time-up payload from WS:", timeUpPayload)
...
//             {/* Replace Avatar ID input with 3D Avatar selector */}
//             <div>
//               <p className="text-white text-sm mb-2 text-center">Choose your avatar</p>
//               <div className="bg-white rounded-2xl p-3">
//                 {/* AvatarSelector will call onSelect with avatarId (number) */}
//                 {/* Pass empty userId so it doesn't POST to /api/users before join */}
//                 {/* eslint-disable-next-line @typescript-eslint/ban-ts-comment */}
//                 {/* @ts-ignore */}
//                 <AvatarSelector userId={""} onSelect={(id: number) => setAvatarId(String(id))} />
//               </div>
//               <p className="text-white/70 text-xs mt-2 text-center">Click an avatar to select it. The avatar ID will be used when joining.</p>
//             </div>
      ...
      <div className="max-w-4xl w-full space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          >
            <p className="text-green-200 text-sm mb-1">🎯 Accuracy</p>
            <p className="text-white text-3xl font-bold">{analytics.accuracyRate.toFixed(0)}%</p>
            <p className="text-green-300 text-xs mt-1">{analytics.correctAnswers} got it right</p>
          </motion.div>
        </div>
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-semibold ${option.isCorrect ? "text-green-300" : "text-white"}`}>
                      {option.optionText}
                    </span>
                  </div>
                  <span className="text-white text-sm font-bold">
            ...
            <div className="grid grid-cols-3 gap-3">
              {analytics.top3.map((entry, index) => (
                <motion.div
                  ...
                  transition={{ delay: 0.7 + index * 0.1, type: "spring" }}
                  className={`p-4 rounded-lg text-center ${
                    entry.participantId === participantId ? "bg-yellow-500/30 border-2 border-yellow-400" : "bg-white/5"
                  }`}
                >
                  <div className="text-3xl mb-1">{index === 0 ? "🥇" : index === 1 ? "🥈" : "🥉"}</div>
                  <p className="text-white font-bold text-sm truncate">{entry.nickname}</p>
                  <p className="text-white text-lg font-bold">{entry.totalScore}</p>
                </motion.div>
              ))}
        ...
        const subscriptions = [
          ...
          stomp.subscribe(`/user/queue/session/${quizCode}/time-up`, (msg) => {
            ...
            if (timeUpMessage) {
              ...
              // Notify parent via callback
              try {
                onTimeUpRef.current?.(timeUpMessage)
              } catch (e) {
                console.warn("onTimeUp handler failed", e)
              }
            }
          }),
...
export default function ParticipantQuizFixed() {
  ...
  const handleJoinSuccess = (participantData: {
    id: string
    nickname: string
    avatarId: string
    totalScore: number
    ...
    setNickname(participantData.nickname)
    setAvatarId(participantData.avatarId)
    setPersonalScore(participantData.totalScore)
    setJoined(true)
  }
  ...
  // Join form UI
  if (!joined) {
    return <JoinFlow sessionCode={sessionCode} onJoinSuccess={handleJoinSuccess} />
  }
  ...
  // Show analytics after answer reveal
  if (showAnalytics && questionAnalytics) {
    return <ParticipantQuestionAnalytics analytics={questionAnalytics} participantId={participantId} />
  }
