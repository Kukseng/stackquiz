        ...
        <AnimatePresence>
          {leaderboard.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex justify-between items-center relative z-10">
                    <div className="flex items-center gap-4">
                      <div className={`flex items-center justify-center w-16 h-16 rounded-2xl font-black text-2xl shadow-lg ${
                        index === 0 ? "bg-gradient-to-br from-yellow-400 to-yellow-600 text-white" :
                        index === 1 ? "bg-gradient-to-br from-gray-300 to-gray-500 text-white" :
                        index === 2 ? "bg-gradient-to-br from-orange-400 to-orange-600 text-white" :
                        "bg-gradient-to-br from-indigo-500 to-purple-600 text-white"
                      }`}>
                        {medalEmoji || `${index + 1}${getRankSuffix(index + 1)}`}
                      </div>

                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-black text-xl text-gray-800">{entry.nickname}</span>
                    ...
                    <div className="text-right">
                      <div className="flex items-center gap-2">
                        ...
                        {positionChange > 0 && (
                          <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-green-500 text-sm font-bold">
                            ↗️ +{positionChange}
                          </motion.span>
                        )}
                        {positionChange < 0 && (
                          <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-red-500 text-sm font-bold">
                            ↘️ {positionChange}
                          </motion.span>
                        )}
                      </div>
            ...
            <div className="text-center py-12">
              <div className="text-6xl mb-4">👋</div>
              <p className="text-gray-400 text-lg font-semibold">Waiting for participants...</p>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-gray-800 text-sm">{participant.nickname}</span>
    ...
    <div className="bg-white rounded-3xl p-6 shadow-2xl border-4 border-purple-200">
      ...
      <div className="space-y-3">
        ...
        <div className="p-4 bg-gray-50 rounded-2xl border-2 border-gray-200">
          <label className="block text-sm font-bold text-gray-700 mb-2">Set Question Time Limit</label>
  ...
  const fetchHostDashboardByCode = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAuthError(`Failed to load dashboard: ${response.status} ${response.statusText}`)
      }
    } catch (error) {
    ...
    async (command: string, data?: any) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setAuthError(`Failed to execute command ${command}: ${response.status}`)
        }
      } catch (error) {
        setAuthError(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {hostDashboard && (
                    <div className="text-sm bg-purple-100 text-purple-700 px-3 py-1 rounded-full font-semibold">
                      Q{hostDashboard.currentQuestion}/{hostDashboard.totalQuestions} • {hostDashboard.participantsAnswered}/{hostDashboard.totalParticipants} answered
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-indigo-600 px-8 py-4 rounded-2xl shadow-xl border-4 border-white">
              <p className="text-xs text-white/90 font-semibold mb-1 text-center">SESSION CODE</p>
              <p className="text-3xl lg:text-4xl font-black text-white tracking-wider">{sessionCode}</p>
            </div>
