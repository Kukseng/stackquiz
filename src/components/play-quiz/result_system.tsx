...
export function ResultsSystem({
  ...
  const percentage =
    totalPossiblePoints > 0
      ? Math.round((results.score / totalPossiblePoints) * 100)
      : 0;
  const averageTimePerQuestion = Math.round(
    results.timeSpent / results.totalQuestions / 1000
  );

  const { playClick } = useAudio(); // 🔊 audio
  ...
  const getPerformanceMessage = () => {
    ...
    if (percentage >= 60)
      return { message: "Good effort! 👍", color: "text-purple-600" };
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <Badge variant="outline">
                        {answer.correct ? `+${question?.points || 0}` : "0"} pts
                      </Badge>
